...
export function SiteProductGallery({ imageUrls, name }: Props) {
  ...
  const shell = 'site-card relative overflow-hidden bg-surface-muted'
    ...
    <div className={shell}>
      <div className="relative aspect-[16/10] w-full min-h-[260px]">
        ...
        <button
          type="button"
          aria-label="Önceki"
          className="site-btn-icon-lg pointer-events-auto absolute left-3 top-1/2 z-10 -translate-y-1/2 bg-white/95 backdrop-blur-sm"
        ...
        <div className="site-panel absolute right-4 top-4 z-10 bg-white/95 font-semibold tabular-nums backdrop-blur-sm">
          {idx + 1} / {list.length}
        </div>
      </div>
    </div>
